...
import { createObservable, Observable, OB_FLAG } from "src/observable"
...
describe("Observable", () => {
  it("only-next pipe & subscribe", async () => {
    return new Promise((resolve, reject) => {
      ...
      const o1 = new Observable().pipe(v => 1)
      ...
      const o2 = new Observable(false).pipe(
        v => 1,
        v => true
      )
  ...
  it("option-next pipe & subscribe", () => {
    return new Promise<void>((resolve, reject) => {
      ...
      const o1 = new Observable().pipe({ next: v => 1 })
      o1.subscribe({
        next: (v, o) => {
          ary.push(1)
          expect(v).toBe(1)
  ...
  it("emit next/error value", () => {
    return new Promise<void>(resolve => {
      const ob = new Observable(1).pipe(v => v + 1, {
        next: v => v + 1
      })
      ob.subscribe(v => {
        expect(v).toBe(12)
  ...
  it("emit next/error/complete resolve Promise", () => {
    const ob = new Observable(1).pipe(v => ++v)
  ...
  it("unsubscribe", async () => {
    const fn = vi.fn()
    const ob = new Observable().pipe(v => 10)
  ...
  it("pipe skip", async () => {
    const sub = vi.fn()
    const ob = new Observable().pipe(() => OB_FLAG.SKIP)
    ob.subscribe({
      next: sub,
      error: sub,
      complete: sub
